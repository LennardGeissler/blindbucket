...
| **Object-name encryption (`names.encrypt`)** | Off by default. With it on, `PutObject`, `GetObject`, `HeadObject`, `DeleteObject` and listing are served; multipart, copy and tagging return `NotImplemented` until each has an answer to what an encrypted name means. `blindbucket rotate` and `gc` both work. | M6, in progress |
| **Listings under name encryption** | Served for a prefix whose whole result comes back in one page: the gateway decrypts and sorts it, because the provider orders by the encrypted key and an unsorted listing makes `aws s3 sync --delete` delete objects that exist. A prefix larger than one page, a pagination token, a prefix not ending on `/`, or a delimiter other than `/` are refused with `NotImplemented` rather than answered in an unusable order. The buffered tier that would lift the first two is the open half of [ADR-017](adr/ADR-017-listing-order-under-name-encryption.md). | M6, in progress |
