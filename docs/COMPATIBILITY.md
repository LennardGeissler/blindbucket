...
| **Object-name encryption (`names.encrypt`)** | Off by default. With it on, `PutObject`, `GetObject`, `HeadObject`, `DeleteObject` and listing are served; multipart, copy and tagging return `NotImplemented` until each has an answer to what an encrypted name means. | M6, in progress |
